...
def mmd_loss_multiscale(dist1: torch.Tensor, dist2: torch.Tensor, widths_exponents: list) -> torch.Tensor:
    """
    计算两个分布之间的多尺度最大均值差异（MMD）损失
    
    参数:
        dist1: torch.Tensor - 第一个分布的样本张量，形状为(batch_size, feature_dim)
        dist2: torch.Tensor - 第二个分布的样本张量，形状必须与dist1相同
        widths_exponents: list - 多尺度核的宽度和指数参数列表，格式为[(C1, a1), (C2, a2), ...]
    
    返回:
        torch.Tensor - 标量张量，表示多尺度MMD损失值，支持反向传播
    """
    # 检查输入维度是否匹配
    assert dist1.shape == dist2.shape, "两个分布的形状必须完全匹配"
    
    # 计算点积矩阵
    xx = torch.mm(dist1, dist1.t())
    yy = torch.mm(dist2, dist2.t())
    xy = torch.mm(dist1, dist2.t())
