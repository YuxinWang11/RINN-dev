...
config = {
    ...
    "training_params": {
        ...
        "loss_weights": {
            "y_loss": 20.0,#1
            "forward_mmd_loss": 50.0,
            "backward_mmd_loss": 500.0,
...
from R_INN_model.loss_methods import mmd_loss, mmd_loss_multiscale, nmse_loss, weighted_nmse_loss
...
for name, module in model.named_modules():
    if 'actnorm' in name or 'ActNorm' in name:
        if hasattr(module, 'scale'):
            module.scale.data.fill_(1.0)  # 设置scale为1
        if hasattr(module, 'bias'):
            module.bias.data.fill_(0.0)   # 设置bias为0
        if hasattr(module, '_initialized'):
            module._initialized = torch.tensor(True, device=device)  # 标记为已初始化
...
# 损失权重
weight_y = config['training_params']['loss_weights']['y_loss']  # 前向拟合损失权重：1.0
weight_x = config['training_params']['loss_weights']['backward_mmd_loss']  # 反向MMD损失权重：500.0
weight_z = config['training_params']['loss_weights']['forward_mmd_loss']  # 前向MMD损失权重：50.0
weight_reconstruct = config['training_params']['loss_weights'].get('reconstruction_loss', 1.0)  # 重构损失权重：1.0
...
# 训练历史
train_losses = {'total': [], 'y_loss': [], 'forward_mmd_loss': [], 'backward_mmd_loss': [], 'reconstruction_loss': []}
val_losses = {'total': [], 'y_loss': [], 'forward_mmd_loss': [], 'backward_mmd_loss': [], 'reconstruction_loss': []}
...
training_info = {
    ...
    'training_params': {
        ...
        'loss_weights': {
            'y_loss': weight_y,
            'backward_mmd_loss': weight_x,
            'forward_mmd_loss': weight_z,
            'reconstruction_loss': weight_reconstruct
        }
    },
    'data_info': {
...
def calculate_loss(left_input, right_input):
    """计算正确的RINN损失，借鉴train.py中的损失函数结构
    核心：
    1. 正向拟合损失：逐点预测准确，使用L2距离
    2. 前向MMD损失：分布匹配，使用多尺度MMD
    3. 反向MMD损失：重构质量，使用多尺度MMD
    4. 重构损失：噪声鲁棒性，使用L2距离（加噪）
    """
    ...
    # 1. 正向拟合损失：逐点预测准确，L2距离
    l_forw_fit = torch.mean((predicted_y - real_y) ** 2)
    
    # 2. 前向MMD损失：分布匹配，多尺度MMD
    # 截断y部分的梯度，与train.py保持一致
    # 构造输出：z部分 + 截断梯度的y部分
    output_block_grad = torch.cat((predicted_z, predicted_y.data), dim=1)
    # 构造目标：真实z部分 + 真实y部分
    real_z = right_input[:, y_dim:]
    y_short = torch.cat((real_z, real_y), dim=1)
    # 定义多尺度核参数
    mmd_forw_kernels = [(2.0, 1.0), (5.0, 0.5), (10.0, 0.25)]
    l_forw_mmd = mmd_loss_multiscale(output_block_grad, y_short, mmd_forw_kernels)
    
    # 3. 反向MMD损失：重构质量，多尺度MMD
    # 反向映射：使用实际的Y和Z进行逆向预测
    # 从right_input中提取真实Y和Z
    real_z = right_input[:, y_dim:]
    # 使用真实的Y和Z进行反向映射
    real_yz = right_input  # right_input = Y + Z
    reconstructed_left, log_det_backward = model.inverse(real_yz)
    reconstructed_x = reconstructed_left[:, :x_dim]
    
    # 定义多尺度核参数
    mmd_back_kernels = [(2.0, 1.0), (5.0, 0.5), (10.0, 0.25)]
    l_back_mmd = mmd_loss_multiscale(real_x, reconstructed_x, mmd_back_kernels)
    
    # 4. 重构损失：使用原始预测值进行反向映射，不加噪声
    # 使用原始的predicted_y和predicted_z进行反向映射
    reconstructed_right_input = torch.cat((predicted_z, predicted_y), dim=1)
    reconstructed_left_from_pred, _ = model.inverse(reconstructed_right_input)
    reconstructed_x_from_pred = reconstructed_left_from_pred[:, :x_dim]
    
    l_reconstruct = torch.mean((reconstructed_x_from_pred - real_x) ** 2)
    
    # 总损失：组合四个损失项
    total_loss = weight_y * l_forw_fit + weight_z * l_forw_mmd + weight_x * l_back_mmd + weight_reconstruct * l_reconstruct
    
    return {
        "total_loss": total_loss,
        "y_loss": l_forw_fit,
        "forward_mmd_loss": l_forw_mmd,
        "backward_mmd_loss": l_back_mmd,
        "reconstruction_loss": l_reconstruct
    }
...
if not skip_training:
    ...
    for epoch in range(num_epochs):
        ...
        # 每次epoch重新生成z并创建新的DataLoader
        print(f'\nEpoch {epoch+1}/{num_epochs}: 生成新的z...')
        ...
        # 训练阶段
        model.train()
        epoch_train_losses = {"total_loss": 0.0, "y_loss": 0.0, "forward_mmd_loss": 0.0, "backward_mmd_loss": 0.0, "reconstruction_loss": 0.0}
        ...
        train_losses['forward_mmd_loss'].append(epoch_train_losses['forward_mmd_loss'])
        train_losses['backward_mmd_loss'].append(epoch_train_losses['backward_mmd_loss'])
        train_losses['reconstruction_loss'].append(epoch_train_losses['reconstruction_loss'])
        ...
        print(f"Epoch [{epoch+1}/{num_epochs}], "
              ...
              f"Forward MMD Loss: {epoch_train_losses['forward_mmd_loss']:.6f}, "
              f"Backward MMD Loss: {epoch_train_losses['backward_mmd_loss']:.6f}, "
              f"Reconstruction Loss: {epoch_train_losses['reconstruction_loss']:.6f}, "
        ...
        model.eval()
        epoch_val_losses = {"total_loss": 0.0, "y_loss": 0.0, "forward_mmd_loss": 0.0, "backward_mmd_loss": 0.0, "reconstruction_loss": 0.0}
        ...
        print(f"          Val Loss: {epoch_val_losses['total_loss']:.6f}, "
              ...
              f"Forward MMD Loss: {epoch_val_losses['forward_mmd_loss']:.6f}, "
              f"Backward MMD Loss: {epoch_val_losses['backward_mmd_loss']:.6f}, "
              f"Reconstruction Loss: {epoch_val_losses['reconstruction_loss']:.6f}, "
              f"Time: {val_time:.2f}s")
        
        # 记录验证历史
        val_losses['total'].append(epoch_val_losses['total_loss'])
        val_losses['y_loss'].append(epoch_val_losses['y_loss'])
        val_losses['forward_mmd_loss'].append(epoch_val_losses['forward_mmd_loss'])
        val_losses['backward_mmd_loss'].append(epoch_val_losses['backward_mmd_loss'])
        val_losses['reconstruction_loss'].append(epoch_val_losses['reconstruction_loss'])
    ...
    train_losses['forward_mmd_loss'] = [0.5, 0.4, 0.3, 0.2, 0.1] * 20
    train_losses['backward_mmd_loss'] = [0.4, 0.3, 0.2, 0.1, 0.1] * 20
    train_losses['reconstruction_loss'] = [0.3, 0.2, 0.2, 0.1, 0.1] * 20
    val_losses['total'] = [1.2, 0.9, 0.7, 0.5, 0.3] * 20
    val_losses['y_loss'] = [0.9, 0.7, 0.5, 0.4, 0.3] * 20
    val_losses['forward_mmd_loss'] = [0.6, 0.5, 0.4, 0.3, 0.2] * 20
    val_losses['backward_mmd_loss'] = [0.5, 0.4, 0.3, 0.2, 0.2] * 20
    val_losses['reconstruction_loss'] = [0.4, 0.3, 0.3, 0.2, 0.2] * 20
    best_val_loss = 0.2
...
axes[1].set_title('Y Loss (L2 Fit)')
axes[1].legend()
axes[1].grid(True, alpha=0.3)

# Forward MMD loss curve
axes[2].plot(train_losses['forward_mmd_loss'], label='Train Forward MMD', color='blue')
axes[2].plot(val_losses['forward_mmd_loss'], label='Val Forward MMD', color='red')
axes[2].set_xlabel('Epoch')
axes[2].set_ylabel('Loss')
axes[2].set_title('Forward MMD Loss')
axes[2].legend()
axes[2].grid(True, alpha=0.3)

# Backward MMD loss curve
axes[3].plot(train_losses['backward_mmd_loss'], label='Train Backward MMD', color='blue')
axes[3].plot(val_losses['backward_mmd_loss'], label='Val Backward MMD', color='red')
axes[3].set_xlabel('Epoch')
axes[3].set_ylabel('Loss')
axes[3].set_title('Backward MMD Loss')
axes[3].legend()
axes[3].grid(True, alpha=0.3)

# Reconstruction loss curve
axes[4].plot(train_losses['reconstruction_loss'], label='Train Reconstruction', color='blue')
axes[4].plot(val_losses['reconstruction_loss'], label='Val Reconstruction', color='red')
