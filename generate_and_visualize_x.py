...
MODEL_DIR = 'model_checkpoints_rinn/rinn_correct_structure_20260308_142340'
...
# 对每个生成的X样本进行前向预测，验证是否能恢复原始Y
reconstructed_ys = []
y_recovery_errors = []

for i, x_sample in enumerate(reconstructed_xs):
    # 截取x的前五维度
    x_trimmed = x_sample[:5]
    # 归一化
    x_trimmed_normalized = (x_trimmed - x_mean) / (x_std + 1e-8)
    # 进行197维度的0填充
    padding_dim = left_input_dim - len(x_trimmed)
    x1 = np.concatenate((x_trimmed_normalized, np.zeros(padding_dim, dtype=np.float32)))
    x1_tensor = torch.FloatTensor(x1.reshape(1, -1)).to(device)
    
    # 前向预测得到y1
    with torch.no_grad():
        predicted_right, _, _ = model(x1_tensor, return_intermediate=True)
        y1_normalized = predicted_right[:, :len(y_normalized)]
        y1 = y1_normalized.cpu().numpy()[0] * y_std + y_mean
    
    reconstructed_ys.append(y1)
    
    # 计算y1与y0的误差
    error = np.mean(np.abs(y1 - y_data))
    y_recovery_errors.append(error)
    
    print(f'样本 {i+1} Y恢复误差: {error:.6f}')

reconstructed_ys = np.array(reconstructed_ys)
y_recovery_errors = np.array(y_recovery_errors)
print(f'平均Y恢复误差: {y_recovery_errors.mean():.6f}')

# ============== 可逆性验证2：y0 -> x -> y ==============
print('\n=== 可逆性验证2：y0 -> x -> y ===')
print('验证给定y0，逆向得到x，再将x输入模型得到y，验证y是否等于y0')

# 使用第一个z样本进行验证
z_test = z_samples[0:1]  # 取第一个z样本
y_test = y_normalized.reshape(1, -1)  # 原始y0

# 构造右侧输入：y0 + z
y_padding_test = np.zeros((1, y_padding_dim), dtype=np.float32)
right_input_test = np.concatenate((y_test, z_test, y_padding_test), axis=1)
right_input_test_tensor = torch.FloatTensor(right_input_test).to(device)

# 逆向映射：y0 + z -> x
with torch.no_grad():
    reconstructed_left_test, _ = model.inverse(right_input_test_tensor)
    x_from_y_normalized = reconstructed_left_test[:, :x_dim]
    x_from_y = x_from_y_normalized.cpu().numpy() * x_std + x_mean

print(f'从y0逆向得到的x: {x_from_y[0]}')

# 前向映射：x -> y
padding_dim = left_input_dim - x_dim
left_input_test = np.concatenate((x_from_y_normalized.cpu().numpy(), np.zeros((1, padding_dim), dtype=np.float32)), axis=1)
left_input_test_tensor = torch.FloatTensor(left_input_test).to(device)

with torch.no_grad():
    predicted_right_test, _, _ = model(left_input_test_tensor, return_intermediate=True)
    y_reconstructed_normalized = predicted_right_test[:, :len(y_normalized)]
    y_reconstructed = y_reconstructed_normalized.cpu().numpy() * y_std + y_mean

# 计算误差
y0_reconstructed_error = np.mean(np.abs(y_reconstructed[0] - y_data))
y0_reconstructed_mse = np.mean((y_reconstructed[0] - y_data) ** 2)
y0_reconstructed_nmse = np.sum((y_reconstructed[0] - y_data) ** 2) / np.sum(y_data ** 2)

print(f'重新前向得到的y与y0的绝对误差: {y0_reconstructed_error:.6f}')
print(f'重新前向得到的y与y0的MSE: {y0_reconstructed_mse:.6f}')
print(f'重新前向得到的y与y0的NMSE: {y0_reconstructed_nmse:.6f}')

# 分离实部和虚部进行详细分析
y_data_real = y_data[:len(real_data)]
y_data_imag = y_data[len(real_data):]
y_reconstructed_real = y_reconstructed[0][:len(real_data)]
y_reconstructed_imag = y_reconstructed[0][len(real_data):]

real_error = np.mean(np.abs(y_reconstructed_real - y_data_real))
imag_error = np.mean(np.abs(y_reconstructed_imag - y_data_imag))

print(f'实部平均绝对误差: {real_error:.6f}')
print(f'虚部平均绝对误差: {imag_error:.6f}')
...
# 对每个样本的恢复结果进行可视化
for i, y_reconstructed in enumerate(reconstructed_ys):
    y_reconstructed_real = y_reconstructed[:len(real_data)]
    y_reconstructed_imag = y_reconstructed[len(real_data):]
    
    # 计算S11幅度
    y_data_s11 = np.sqrt(y_data_real**2 + y_data_imag**2)
    y_reconstructed_s11 = np.sqrt(y_reconstructed_real**2 + y_reconstructed_imag**2)
    ...
    # 实部对比
    plt.subplot(3, 1, 1)
    plt.plot(freq_data, y_data_real, label='Original Re(S11)', color='blue', linewidth=2)
    plt.plot(freq_data, y_reconstructed_real, label=f'Reconstructed Re(S11) (Sample {i+1})', color='red', linestyle='--', linewidth=2)
    plt.xlabel('Frequency (GHz)')
    plt.ylabel('Re(S11)')
    plt.title(f'Comparison of Original and Reconstructed Re(S11) - Sample {i+1}')
    plt.legend()
    plt.grid(True, alpha=0.3)
    
    # 虚部对比
    plt.subplot(3, 1, 2)
    plt.plot(freq_data, y_data_imag, label='Original Im(S11)', color='green', linewidth=2)
    plt.plot(freq_data, y_reconstructed_imag, label=f'Reconstructed Im(S11) (Sample {i+1})', color='orange', linestyle='--', linewidth=2)
    plt.xlabel('Frequency (GHz)')
    plt.ylabel('Im(S11)')
    plt.title(f'Comparison of Original and Reconstructed Im(S11) - Sample {i+1}')
    plt.legend()
    plt.grid(True, alpha=0.3)
    
    # S11幅度对比
    plt.subplot(3, 1, 3)
    plt.plot(freq_data, y_data_s11, label='Original |S11|', color='purple', linewidth=2)
    plt.plot(freq_data, y_reconstructed_s11, label=f'Reconstructed |S11| (Sample {i+1})', color='brown', linestyle='--', linewidth=2)
    plt.xlabel('Frequency (GHz)')
    plt.ylabel('|S11|')
    plt.title(f'Comparison of Original and Reconstructed |S11| - Sample {i+1}')
    plt.legend()
    plt.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, f'y_reconstruction_sample_{i+1}.png'), dpi=150, bbox_inches='tight')
    plt.close()
    print(f'Y恢复结果可视化已保存: y_reconstruction_sample_{i+1}.png')
